...
namespace Texample
{
    ...
    {
        //--Constants--//
        public static readonly int CHAR_START = 32;           // First Character (ASCII Code)
        public static readonly int CHAR_END = 126;            // Last Character (ASCII Code)
        public static readonly int CHAR_CNT = (((CHAR_END - CHAR_START) + 1) + 1);  // Character Count (Including Character to use for Unknown)

        public static readonly int CHAR_NONE = 32;            // Character to Use for Unknown (ASCII Code)
        public static readonly int CHAR_UNKNOWN = (CHAR_CNT - 1);  // Index of the Unknown Character

        public static readonly int FONT_SIZE_MIN = 6;         // Minumum Font Size (Pixels)
        public static readonly int FONT_SIZE_MAX = 180;       // Maximum Font Size (Pixels)

        public static readonly int CHAR_BATCH_SIZE = 100;     // Number of Characters to Render Per Batch
        ...
        readonly float[] charWidths;                       // Width of Each Character (Actual; Pixels)
        ...
        public bool load(String file, int size, int padX, int padY) {
            ...
            paint.AntiAlias = true;                       // Enable Anti Alias
            paint.TextSize = size;                        // Set Text Size
            paint.Color = new Color(255, 255, 255, 255);          // Set ARGB (White, Opaque)
            paint.SetTypeface(tf);                        // Set Typeface

            // get font metrics
            Paint.FontMetrics fm = paint.GetFontMetrics();  // Get Font Metrics
            fontHeight = (float)Math.Ceiling(Math.Abs(fm.Bottom) + Math.Abs(fm.Top));  // Calculate Font Height
            fontAscent = (float)Math.Ceiling(Math.Abs(fm.Ascent));  // Save Font Ascent
            fontDescent = (float)Math.Ceiling(Math.Abs(fm.Descent));  // Save Font Descent
            ...
            for (char c = (char)CHAR_START; c <= CHAR_END; c++) {  // FOR Each Character
                s[0] = c;                                    // Set Character
            ...
            s[0] = (char)CHAR_NONE;                               // Set Unknown Character
            paint.GetTextWidths(s, 0, 1, w);              // Get Character Bounds
            charWidths[cnt] = w[0];                         // Get Width
            if (charWidths[cnt] > charWidthMax)           // IF Width Larger Than Max Width
                charWidthMax = charWidths[cnt];              // Save New Max Width
            ...
            Bitmap bitmap = Bitmap.CreateBitmap(textureSize, textureSize, Bitmap.Config.Alpha8);  // Create Bitmap
            ...
            rowCnt = (int)Math.Ceiling((float)CHAR_CNT / (float)colCnt);  // Calculate Number of Rows
            ...
            s[0] = (char)CHAR_NONE;                               // Set Character to Use for NONE
            canvas.DrawText(s, 0, 1, x, y, paint);        // Draw Character

            // generate a new texture
            int[] textureIds = new int[1];                  // Array to Get Texture Id
            GL.GenTextures(1, textureIds);           // Generate New Texture
            textureId = textureIds[0];                      // Save Texture Id

            // setup filters for texture
            GL.BindTexture(All.Texture2D, textureId);  // Bind Texture
            GL.TexParameter(All.Texture2D, All.TextureMinFilter, (int)All.Nearest);  // Set Minification Filter
            GL.TexParameter(All.Texture2D, All.TextureMagFilter, (int)All.Linear);  // Set Magnification Filter
            GL.TexParameter(All.Texture2D, All.TextureWrapS, (int)All.ClampToEdge);  // Set U Wrapping
            GL.TexParameter(All.Texture2D, All.TextureWrapT, (int)All.ClampToEdge);  // Set V Wrapping

            // load the generated bitmap onto the texture
            GLUtils.TexImage2D((int)All.Texture2D, 0, bitmap, 0);  // Load Bitmap to Texture
            GL.BindTexture(All.Texture2D, 0);      // Unbind Texture
